...
function ensureUI() {
  ...
  function btn(act, text, cls = '') {
    return `<button type="button" class="perf-lab-btn ${cls}" data-act="${act}">${text}</button>`;
  }
...
function hasMusicLabHint(value) {
  const raw = String(value || '').trim().toLowerCase();
  if (!raw) return false;
  return raw.includes('musiclab') || raw.includes('music-lab') || raw.includes('music lab');
}

function inferLabTypeFromResultsMeta(results, meta = {}) {
  const explicitType = String(meta?.labType || meta?.kind || '').trim();
  if (explicitType) return normalizeLabType(explicitType);

  const rows = Array.isArray(results) ? results : [results];
  for (const row of rows) {
    if (!row || typeof row !== 'object') continue;
    if (row.musicLab && typeof row.musicLab === 'object') return 'music';
  }
...
function buildResultsBundle(results, meta = {}) {
  ...
  const labType = inferLabTypeFromResultsMeta(results, meta);
...
async function fetchWithPerfLabTimeout(url, options = null, timeoutMs = 12000) {
  const ms = Math.max(1000, Math.trunc(Number(timeoutMs) || 12000));
...
async function postResultsBundle(bundle, url, opts = {}) {
  ...
  try {
    const res = await fetchWithPerfLabTimeout(url, {
      method: 'POST',
    ...
    if (allowLegacyPerfFallback && res && res.status === 404) {
      ...
      if (fallbackUrl) {
        try {
          const retry = await fetchWithPerfLabTimeout(fallbackUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(bundle),
          });
...
async function publishResultBundle(result, meta = {}) {
  ...
  const labType = inferLabTypeFromResultsMeta([result], publishMeta);
  ...
  const postUrl = resolveLabPostUrl(cfg, labType);
  if (postUrl) await postResultsBundle(bundle, postUrl);
  return bundle;
}

async function persistMusicLabSaveResultOnLatestPerfResult(saveResult, meta = {}) {
  const result = (lastResult && typeof lastResult === 'object') ? lastResult : null;
  if (!result) return null;
  result.musicLabSaveResult = (saveResult && typeof saveResult === 'object') ? { ...saveResult } : null;
  try {
    if (Array.isArray(lastResults) && lastResults.length > 0) {
      lastResults[lastResults.length - 1] = result;
    }
  } catch {}
  try { setOutput(result); } catch {}
  const cfg = await resolveResultsConfig();
  const bundle = buildResultsBundle([result], {
    queue: meta.queue || cfg.queue || [],
    notes: (meta.notes != null) ? meta.notes : (cfg.notes || ''),
...
async function publishMusicLabSaveDebugResult(entry = {}) {
  const cfg = await resolveResultsConfig();
  const postUrl = resolveLabPostUrl(cfg, 'perf');
  if (!postUrl) return { ok: false, reason: 'no_perf_post_url' };
  const row = {
    label: String(entry?.label || 'music-lab-save-debug'),
    createdAt: new Date().toISOString(),
    ...
    musicLabSaveDebug: {
      ...
      stageCount: Math.max(0, Number(entry?.stageCount) || 0),
      iterationIndex: Math.max(0, Number(entry?.iterationIndex) || 0),
      iterationCount: Math.max(0, Number(entry?.iterationCount) || 0),
    },
  };
  const bundle = buildResultsBundle([row], {
    runId: String(entry?.runId || ''),
    notes: String(entry?.notes || ''),
    runMode: 'manual',
    scenarioName: 'music-lab-save-debug',
    testCategory: 'music-lab-save-debug',
    labType: 'perf',
    kind: 'perf-lab-debug',
  });
...
async function publishBs0MusicRunStageDebugResult(entry = {}) {
  ...
  if (!postUrl) return { ok: false, reason: 'no_perf_post_url' };
  const row = {
    label: String(entry?.label || 'music-lab-run-stage-debug'),
    createdAt: new Date().toISOString(),
    runId: String(entry?.runId || ''),
    musicLabRunStageDebug: {
      ...
      iterationIndex: Math.max(0, Number(entry?.iterationIndex) || 0),
      iterationCount: Math.max(0, Number(entry?.iterationCount) || 0),
      transportRunning: entry?.transportRunning === true,
      beatSwarmActive: entry?.beatSwarmActive === true,
      perfRunTag: String(entry?.perfRunTag || ''),
    },
  };
  const bundle = buildResultsBundle([row], {
    runId: String(entry?.runId || ''),
    notes: String(entry?.notes || ''),
    runMode: 'manual',
    scenarioName: 'music-lab-run-stage-debug',
    testCategory: 'music-lab-run-stage-debug',
    labType: 'perf',
    kind: 'perf-lab-debug',
  });
  const ok = await postResultsBundle(bundle, postUrl);
...
async function publishRunVariantDebugResult(entry = {}) {
  const cfg = await resolveResultsConfig();
  const postUrl = resolveLabPostUrl(cfg, 'perf');
  if (!postUrl) return { ok: false, reason: 'no_perf_post_url' };
  const row = {
    label: String(entry?.label || 'run-variant-debug'),
    createdAt: new Date().toISOString(),
    runId: String(entry?.runId || ''),
    runVariantDebug: {
      stage: String(entry?.stage || ''),
      ...
      frameCount: Math.max(0, Number(entry?.frameCount) || 0),
      warmupMs: Math.max(0, Number(entry?.warmupMs) || 0),
      transportRunning: entry?.transportRunning === true,
      beatSwarmActive: entry?.beatSwarmActive === true,
      perfRunTag: String(entry?.perfRunTag || ''),
  ...
  const bundle = buildResultsBundle([row], {
    ...
    notes: String(entry?.notes || ''),
    runMode: 'manual',
    scenarioName: 'run-variant-debug',
    testCategory: 'run-variant-debug',
    labType: 'perf',
    kind: 'perf-lab-debug',
  });
  const ok = await postResultsBundle(bundle, postUrl);
...
const PERF_RUN_LIFECYCLE_DEBUG_STORAGE_KEY = 'mt_perf_run_lifecycle_debug_v1';

async function publishPerfRunLifecycleDebugResult(entry = {}) {
  const cfg = await resolveResultsConfig();
  const postUrl = resolveLabPostUrl(cfg, 'perf');
  if (!postUrl) return { ok: false, reason: 'no_perf_post_url' };
  const row = {
    label: String(entry?.label || 'perf-run-lifecycle-debug'),
    createdAt: new Date().toISOString(),
    runId: String(entry?.runId || ''),
    perfRunLifecycleDebug: {
      stage: String(entry?.stage || ''),
      sourceLabel: String(entry?.sourceLabel || ''),
      durationMs: Math.max(0, Number(entry?.durationMs) || 0),
      elapsedMs: Math.max(0, Number(entry?.elapsedMs) || 0),
  ...
  const bundle = buildResultsBundle([row], {
    ...
    notes: String(entry?.notes || ''),
    runMode: 'manual',
    scenarioName: 'perf-run-lifecycle-debug',
    testCategory: 'perf-run-lifecycle-debug',
    labType: 'perf',
    kind: 'perf-lab-debug',
  });
  const ok = await postResultsBundle(bundle, postUrl);
...
function stashPerfRunLifecycleDebug(entry = {}) {
  try {
    localStorage.setItem(PERF_RUN_LIFECYCLE_DEBUG_STORAGE_KEY, JSON.stringify({
      ...entry,
...
async function flushPendingPerfRunLifecycleDebug() {
  let raw = '';
  try { raw = String(localStorage.getItem(PERF_RUN_LIFECYCLE_DEBUG_STORAGE_KEY) || ''); } catch {}
  if (!raw) return null;
  try { localStorage.removeItem(PERF_RUN_LIFECYCLE_DEBUG_STORAGE_KEY); } catch {}
  try {
    const parsed = JSON.parse(raw);
    return await publishPerfRunLifecycleDebugResult(parsed);
  } catch {
    return null;
  }
}

function installPerfRunLifecycleDebugHooks() {
  try {
    if (window.__PERF_RUN_LIFECYCLE_DEBUG_HOOKS_INSTALLED) return;
    window.__PERF_RUN_LIFECYCLE_DEBUG_HOOKS_INSTALLED = true;
    const capture = (stage, detail = '') => {
      const ctx = (window.__PERF_ACTIVE_RUN_DEBUG && typeof window.__PERF_ACTIVE_RUN_DEBUG === 'object')
        ? window.__PERF_ACTIVE_RUN_DEBUG
        : null;
      if (!ctx) return;
      stashPerfRunLifecycleDebug({
        label: 'perf-run-lifecycle-debug',
        runId: String(ctx.runId || ''),
        stage,
        sourceLabel: String(ctx.sourceLabel || ''),
        durationMs: Math.max(0, Number(ctx.durationMs) || 0),
        elapsedMs: Math.max(0, Number(((typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now()) - (Number(ctx.wallStartMs) || 0)) || 0),
        frameCount: Math.max(0, Number(ctx.frameCountRef?.()) || 0),
        perfRunTag: String(ctx.perfRunTag || ''),
        transportRunning: !!(typeof isRunning === 'function' && isRunning()),
        beatSwarmActive: !!window.__beatSwarmActive,
        detail: String(detail || ''),
        href: String(location?.href || ''),
        startedAt: String(ctx.startedAt || ''),
      });
    };
    window.addEventListener('beforeunload', () => capture('beforeunload'), { capture: true });
    window.addEventListener('pagehide', () => capture('pagehide'), { capture: true });
    window.addEventListener('error', (event) => {
      capture('error', String(event?.message || event?.error?.message || 'window_error'));
    }, { capture: true });
    window.addEventListener('unhandledrejection', (event) => {
      const reason = event?.reason;
      capture('unhandledrejection', String(reason?.message || reason || 'unhandled_rejection'));
    }, { capture: true });
  } catch {}
}

function publishRunVariantDebugResultAsync(entry = {}) {
  try {
    if (typeof window !== 'undefined' && window.__PERF_LAB_DEFER_DEBUG_POSTS) return;
    Promise.resolve()
      .then(() => publishRunVariantDebugResult(entry))
      .catch(() => {});
  } catch {}
}

try {
  installPerfRunLifecycleDebugHooks();
  Promise.resolve().then(() => flushPendingPerfRunLifecycleDebug()).catch(() => {});
} catch {}
...
async function runAuto(config = {}) {
  ...
  try {
    ...
    const labType = inferLabTypeFromResultsMeta(results, bundleMeta);
...
async function finalizeMusicTraceCaptureForPerfRun(config = null) {
  ...
  try {
    window.__beatSwarmMusicTrace = { enabled: false, include: [], exclude: [], console: false };
  } catch {}
...
async function saveMusicLabSessionToResourcesGlobal({
  ...
  const api = getMusicLabApiGlobal();
  if (!payloadOverride && (!api || typeof api.exportSession !== 'function')) {
    const result = { ok: false, reason: 'music_lab_api_unavailable', postUrl: '', sessionId: '', events: 0, sessionSummary: null };
  ...
  let payload = null;
  try {
    payload = payloadOverride && typeof payloadOverride === 'object'
      ? payloadOverride
      : ((typeof api.exportSessionForSave === 'function')
        ? api.exportSessionForSave({ compact: forceCompactSave === true })
        : api.exportSession());
  ...
  const payloadDebug = {
    ...
    executedEventCount: Array.isArray(payload?.session?.executedEvents) ? payload.session.executedEvents.length : 0,
  ...
  if (!postUrl) {
    const result = {
      ...
      sessionId: String(payload?.sessionId || ''),
      events: Array.isArray(payload?.eventTimeline) ? payload.eventTimeline.length : 0,
      sessionSummary: summarizeMusicLabSessionPayload(payload),
      payloadDebug,
    };
    try { window.__LAST_MUSIC_LAB_SAVE_RESULT = { ...result, runId, label, notes }; } catch {}
    try { console.warn('[PerfLab] Music Lab save failed', { ...result, runId, label, notes }); } catch {}
  ...
  try { window.__LAST_MUSIC_LAB_SAVE_RESULT = { ...result, runId, label, notes }; } catch {}
  try {
    if (ok) console.info('[PerfLab] Music Lab save succeeded', { ...result, runId, label, notes });
    else console.warn('[PerfLab] Music Lab save failed', { ...result, runId, label, notes });
  } catch {}
  return result;
}

function exportMusicLabPayloadForDeferredSave() {
  const api = getMusicLabApiGlobal();
  if (!api || typeof api.exportSession !== 'function') {
    return {
      ok: false,
      reason: 'music_lab_api_unavailable',
      payload: null,
      payloadDebug: null,
    };
  }
  ...
  try {
    payload = (typeof api.exportSessionForSave === 'function')
      ? api.exportSessionForSave({ compact: true })
      : api.exportSession();
...
async function saveDeferredMusicLabBatchToResourcesGlobal({
  runId = 'musicLabBatchAutoSave',
  label = 'music-lab-batch',
  notes = '',
  scenarioName = '',
  testCategory = '',
  forceCompactSave = false,
  ...
  const cfg = await resolveResultsConfig();
  const postUrl = resolveLabPostUrl(cfg, 'music');
  if (!postUrl) return { ok: false, reason: 'no_post_url', postUrl: '', runDetails: [] };
  ...
  for (const item of runItems) {
    ...
    const runIndex = Math.max(1, Math.trunc(Number(args.iterationIndex) || results.length + 1));
    const runIdForItem = String(args.runId || `${runId}_run${runIndex}`).trim() || `${runId}_run${runIndex}`;
    const compacted = forceCompactSave === true ? compactMusicLabPayloadForSave(payload) : { payload, compacted: false, detail: '' };
    const payloadForSave = compacted.payload;
    const sessionSummary = summarizeMusicLabSessionPayload(payload);
    results.push({
      label: String(args.label || `${label}-run${runIndex}`),
      runId: runIdForItem,
      createdAt: new Date().toISOString(),
      iterationIndex: runIndex,
      iterationCount: Math.max(1, Math.trunc(Number(args.iterationCount) || runItems.length || 1)),
      musicLab: payloadForSave,
    });
    runDetails.push({
      runIndex,
  ...
  const aggregateSummary = aggregateMusicRunSummaries(
    runDetails.map((detail) => detail.sessionSummary).filter(Boolean)
  );
  const bundle = buildResultsBundle(results, {
    runId: String(runId || 'musicLabBatchAutoSave'),
    notes: String(notes || ''),
    runMode: 'multi-run',
    scenarioName: String(scenarioName || label || 'music-lab-batch'),
    testCategory: String(testCategory || scenarioName || 'music-lab-batch'),
    iterationIndex: 1,
    iterationCount: Math.max(1, Math.trunc(Number(runItems.length) || 1)),
    labType: 'music',
    kind: 'music-lab-batch',
    saveTimestampIso,
    aggregateSummary,
    runDetails,
  });
...
function waitForPerfLabMs(ms = 0) {
  return new Promise((resolve) => {
    try {
      setTimeout(resolve, Math.max(0, Number(ms) || 0));
    } catch {
...
async function waitForTransportRunning(timeoutMs = 1800) {
  const deadline = Date.now() + Math.max(50, Number(timeoutMs) || 1800);
  while (Date.now() < deadline) {
    try {
      if (isRunning()) return true;
    } catch {}
    await waitForPerfLabMs(50);
  }
  try {
    return !!isRunning();
  } catch {
...
async function runBS0Stage(stageCount = 1, opts = null) {
  ...
  try {
    ...
    for (let runIndex = 1; runIndex <= repeatCount; runIndex += 1) {
      ...
      if (musicLabApi && typeof musicLabApi.setEnabled === 'function') {
        if (prevMusicLabEnabled === null && typeof musicLabApi.isEnabled === 'function') {
          try { prevMusicLabEnabled = musicLabApi.isEnabled() === true; } catch { prevMusicLabEnabled = false; }
        }
        if (
          prevMusicLabRealtimeMetricsEnabled === null
          && typeof musicLabApi.isRealtimeMetricsEnabled === 'function'
        ) {
          try { prevMusicLabRealtimeMetricsEnabled = musicLabApi.isRealtimeMetricsEnabled() === true; } catch { prevMusicLabRealtimeMetricsEnabled = true; }
        }
        if (
          prevMusicLabLightweightSystemEventsEnabled === null
          && typeof musicLabApi.isLightweightSystemEventsEnabled === 'function'
        ) {
          try { prevMusicLabLightweightSystemEventsEnabled = musicLabApi.isLightweightSystemEventsEnabled() === true; } catch { prevMusicLabLightweightSystemEventsEnabled = false; }
        }
        try { musicLabApi.setEnabled(true); } catch {}
        try { musicLabApi.setRealtimeMetricsEnabled?.(cfg.keepMusicLabRealtimeMetrics === true); } catch {}
        try { musicLabApi.setLightweightSystemEventsEnabled?.(true); } catch {}
      }
      if (resetMusicLabEachRun) {
        const api = musicLabApi || getMusicLabApiGlobal();
        if (api && typeof api.reset === 'function') {
      ...
      if (publishPerfArtifacts) {
        try {
          await publishBs0MusicRunStageDebugResult({
            label: `music-lab-run-stage-debug${runSuffix}`,
            runId: `${saveRunIdBase}${runSuffix}`,
            stage: traceCaptureConfig?.enabled === true ? 'before_trace_finalize' : 'trace_finalize_skipped',
            sourceLabel: `${labelPrefix}${runSuffix}`,
            notes: saveNotes || `Auto save for BS0 S${stageCount} run ${runIndex}/${repeatCount}`,
            durationMs,
            stageCount,
      ...
      if (traceCaptureConfig?.enabled === true) {
        const traceFilePrefix = String(
          traceCaptureConfig.fileNamePrefix || `beat-swarm-trace-s${stageCount}${runSuffix}`
        ).trim();
        const traceFileBase = repeatCount > 1 ? `${traceFilePrefix}${runSuffix}` : traceFilePrefix;
      ...
      if (saveMusicLabEachRun) {
        ...
        let save = null;
        try {
          save = deferMusicLabSaveUntilBatchEnd
            ? {
                ok: deferredExport?.ok === true,
                reason: String(deferredExport?.reason || ''),
                postUrl: 'deferred_until_batch_end',
                sessionId: String(deferredExport?.payload?.sessionId || ''),
                events: Array.isArray(deferredExport?.payload?.eventTimeline) ? deferredExport.payload.eventTimeline.length : 0,
                sessionSummary: deferredExport?.payload ? summarizeMusicLabSessionPayload(deferredExport.payload) : null,
                payloadDebug: deferredExport?.payloadDebug || null,
                deferred: true,
              }
            : await saveMusicLabSessionToResourcesGlobal(saveArgs);
        } catch (err) {
          save = {
            ok: false,
            reason: 'save_call_threw',
            error: String(err && err.message || err || 'save_call_threw'),
          ...
          try { window.__LAST_MUSIC_LAB_SAVE_RESULT = { ...save, ...saveArgs }; } catch {}
          try { console.warn('[PerfLab] Music Lab save call threw', { save, saveArgs }); } catch {}
        }
        if (deferMusicLabSaveUntilBatchEnd && save.ok) {
          deferredMusicLabSaves.push({
            args: saveArgs,
            payload: deferredExport.payload,
          });
        }
        runOutcomes.push({
          runIndex,
        ...
        if (!deferMusicLabSaveUntilBatchEnd) try {
          const cfgResolved = await resolveResultsConfig();
    ...
    if (saveMusicLabEachRun) {
      if (deferMusicLabSaveUntilBatchEnd) {
        const postedBatch = await saveDeferredMusicLabBatchToResourcesGlobal({
          runId: groupedRunId,
          label: groupedScenarioName,
          notes: groupedNotes,
          scenarioName: groupedScenarioName,
          testCategory: groupedScenarioName,
          forceCompactSave: cfg.forceCompactSave === true,
          runs: deferredMusicLabSaves,
        });
        for (const detail of (Array.isArray(postedBatch?.runDetails) ? postedBatch.runDetails : [])) {
          const runIndex = Math.max(1, Math.trunc(Number(detail?.runIndex) || 1));
          const existingOutcome = runOutcomes.find((outcome) => Math.max(1, Math.trunc(Number(outcome?.runIndex) || 1)) === runIndex);
          if (!existingOutcome) continue;
          existingOutcome.saved = postedBatch?.ok === true;
          existingOutcome.reason = String(postedBatch?.reason || '');
          existingOutcome.postUrl = String(postedBatch?.postUrl || '');
          existingOutcome.sessionId = String(detail?.sessionId || existingOutcome.sessionId || '');
          existingOutcome.events = Math.max(0, Number(detail?.events) || Number(existingOutcome.events) || 0);
          existingOutcome.sessionSummary = (detail?.sessionSummary && typeof detail.sessionSummary === 'object')
            ? detail.sessionSummary
            : existingOutcome.sessionSummary;
          existingOutcome.deferredBatchPosted = true;
        }
        if (!postedBatch?.ok) {
          setStatus(`BS0 S${stageCount} deferred batch save failed`);
          setOutput({
            ok: false,
            stageCount,
            repeatCount,
            durationMs,
            reason: String(postedBatch?.reason || 'deferred_batch_save_failed'),
            postUrl: String(postedBatch?.postUrl || ''),
          });
          return {
            ok: false,
            stageCount,
      ...
      if (repeatCount > 1 && !deferMusicLabSaveUntilBatchEnd) {
    ...
    if (prevMusicLabEnabled !== null) {
      const api = getMusicLabApiGlobal();
      if (api && typeof api.setEnabled === 'function') {
        try { api.setEnabled(prevMusicLabEnabled); } catch {}
      }
    }
    if (prevMusicLabRealtimeMetricsEnabled !== null) {
      const api = getMusicLabApiGlobal();
      if (api && typeof api.setRealtimeMetricsEnabled === 'function') {
        try { api.setRealtimeMetricsEnabled(prevMusicLabRealtimeMetricsEnabled); } catch {}
      }
    }
    if (prevMusicLabLightweightSystemEventsEnabled !== null) {
      const api = getMusicLabApiGlobal();
      if (api && typeof api.setLightweightSystemEventsEnabled === 'function') {
        try { api.setLightweightSystemEventsEnabled(prevMusicLabLightweightSystemEventsEnabled); } catch {}
      }
    }
  }
}
...
async function runVariantPlaying(label, step, statusText, options = null) {
  ...
  setOutput(null);
  lastResult = null;
  try { window.__PERF_LAB_DEFER_DEBUG_POSTS = true; } catch {}
  ...
  try {
    try {
      window.__PERF_ACTIVE_RUN_DEBUG = {
        runId: String(fullLabel || ''),
        sourceLabel: fullLabel,
        durationMs,
        perfRunTag: String(window.__PERF_RUN_TAG || ''),
        startedAt: new Date().toISOString(),
        wallStartMs,
        frameCountRef: () => runVariantFrameCount,
      };
    } catch {}
  ...
  const result = await new Promise((resolve, reject) => {
    ...
    watchdogTimer = setTimeout(() => {
      ...
      publishRunVariantDebugResultAsync({
        label: `run-variant-debug__${fullLabel}`,
        runId: String(fullLabel || ''),
        stage: 'watchdog_timeout',
        sourceLabel: fullLabel,
        durationMs,
        elapsedMs,
        frameCount: runVariantFrameCount,
        warmupMs,
        transportRunning: isRunning(),
        beatSwarmActive: !!window.__beatSwarmActive,
        perfRunTag: String(window.__PERF_RUN_TAG || ''),
      });
      settleReject(new Error(`runVariantPlaying watchdog timeout after ${elapsedMs}ms`));
      ...
      const stepFrame = (ts) => {
        ...
        raf(stepFrame);
          ...
          publishRunVariantDebugResultAsync({
            ...
            sourceLabel: fullLabel,
            durationMs,
            elapsedMs: Math.max(0, nowMs() - wallStartMs),
            frameCount: runVariantFrameCount,
            warmupMs,
            transportRunning: isRunning(),
            beatSwarmActive: !!window.__beatSwarmActive,
            perfRunTag: String(window.__PERF_RUN_TAG || ''),
            notes: String(err && err.message || err || 'unknown_error'),
          });
          settleReject(err);
        }
      };
  ...
  // BS0 music-lab runs can defer this until after the session export completes.
  if (stopTransportAtEnd) {
    try { stopTransport(); } catch {}
  }
  ...
  try {
    ...
    result.playing = true;
    result.musicLabSaveResult = (window.__LAST_MUSIC_LAB_SAVE_RESULT && typeof window.__LAST_MUSIC_LAB_SAVE_RESULT === 'object')
      ? { ...window.__LAST_MUSIC_LAB_SAVE_RESULT }
      : null;
    let dgPlayheadSeparateEffectiveKnown = 0;
      let dgPlayheadSeparateEffectiveOn = 0;
      let dgTierKnown = 0;
