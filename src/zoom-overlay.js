// zoom-overlay.js — debug build (<300 lines)
// Centers via CSS Grid; neutralizes panel offsets; clamps Grid height;
// INLINE sizes body with !important (beats CSS), clears aspect-ratio;
// emits rich diagnostics to console as [zoom-diag] groups.
...
const VIEWPORT_FRACTION=0.96, MAXW_SQUARE=1200, MAXW_GRID=960;
const GRID_MAX_VH_FRAC=0.34; // cap Grid height vs viewport
const SAFE_PAD_X=28, SAFE_PAD_TOP=28, SAFE_PAD_BOTTOM=64, SQUARE_EPS=0.03;
...
function _isGrid(p){ if(!p) return false; const dt=(p.dataset?.toy||p.getAttribute('data-toy')||'').toLowerCase(); return dt.includes('grid')||dt.includes('loopgrid')||!!p.querySelector('.grid-canvas'); }
...
function _contentBox(header, footer, panel){
  ...
  return {maxW:Math.max(0,maxW), maxH:Math.max(0,maxH), hH, hF, vw:vw|0, vh:vh|0};
}
...
function _fitBody(aspect, box, forceSquare, panel){
  ...
  if (_isGrid(panel)){
    ...
    return {w:Math.max(0,w), h:Math.max(0,h), reason:'grid clamp', steps, maxGridH};
  }
  if (forceSquare){ const side=Math.floor(Math.max(0, Math.min(usableW, usableH))); return {w:side, h:side, reason:'force square'}; }
  let w=Math.min(usableW, MAXW_SQUARE);
  let h=Math.floor(w/Math.max(0.0001, aspect));
  if (h>usableH){ h=usableH; w=Math.floor(h*aspect); }
  return {w:Math.max(0,w), h:Math.max(0,h), reason:'aspect fit'};
}
...
function _applyLayout(panel, header, body, footer, bodyW, bodyH){
  ...
  // Beat stylesheet: height/width + aspect-ratio reset with !important
  body.style.setProperty('width', _px(bodyW), 'important');
  body.style.setProperty('height', _px(bodyH), 'important');
  body.style.setProperty('aspect-ratio', 'auto', 'important');
  body.style.overflow='hidden'; body.style.position='relative'; body.style.zIndex='0'; body.style.boxSizing='border-box';
  if (footer){ footer.style.gridRow='3'; footer.style.alignSelf='start'; footer.style.width='100%'; footer.style.pointerEvents='auto'; footer.style.zIndex='3'; }
}
function _normalizeFooter(panel){
  const vw=panel.querySelector('.toy-volwrap'); const snapVw=_snap(vw);
  if (vw){
    vw.style.position='static'; vw.style.left=''; vw.style.right=''; vw.style.top=''; vw.style.bottom='';
    vw.style.width='100%'; vw.style.maxWidth='100%'; vw.style.pointerEvents='auto'; vw.style.margin='0'; vw.style.gridRow='3'; vw.style.zIndex='4';
  }
...
function _wireMute(panel){
  try{
    ...
    function dispatchVol(pct){ const v=Math.max(0,Math.min(1,(parseInt(pct,10)||0)/100)); try{ window.dispatchEvent(new CustomEvent('toy-volume',{detail:{toyId:idBase,value:v}})); }catch{} }
    rng.addEventListener('input', ()=>{ const p=parseInt(rng.value,10)||0; if(p>0) last=p; }, {passive:true});
    btn.addEventListener('click',(e)=>{ e.stopPropagation(); const muted=btn.getAttribute('aria-pressed')==='true';
...
function _neutraliseOffsets(el){
  const prior={ transform:el.style.transform||null, translate:el.style.translate||null, margin:el.style.margin||null, left:el.style.left||null, top:el.style.top||null, right:el.style.right||null, bottom:el.style.bottom||null, transformOrigin:el.style.transformOrigin||null };
  el.style.transform='none'; try{ el.style.translate='0 0'; }catch{} el.style.margin='0'; el.style.left=''; el.style.top=''; el.style.right=''; el.style.bottom=''; el.style.transformOrigin='50% 50%'; return prior;
}
...
export function ensureOverlay(){
  ...
  if(!overlayEl){
    ...
    Object.assign(overlayEl.style,{ position:'fixed', inset:'0', display:'none', zIndex:'9999', background:'rgba(0,0,0,0.35)', backdropFilter:'blur(2px)', pointerEvents:'none', overflow:'hidden', boxSizing:'border-box' });
  ...
  if(!frameEl || frameEl.parentNode!==overlayEl){
    ...
    Object.assign(frameEl.style,{ position:'absolute', inset:'0', display:'grid', placeItems:'center', width:'100%', height:'100%', pointerEvents:'auto', boxSizing:'border-box',
      paddingTop:`max(${SAFE_PAD_TOP}px, env(safe-area-inset-top))`, paddingRight:`max(${SAFE_PAD_X}px, env(safe-area-inset-right))`, paddingBottom:`max(${SAFE_PAD_BOTTOM}px, env(safe-area-inset-bottom))`, paddingLeft:`max(${SAFE_PAD_X}px, env(safe-area-inset-left))` });
...
export function zoomInPanel(panel, onExit){
  ...
  const isGrid=_isGrid(panel);
  const header=_findHeader(panel); const body=_findBody(panel); const footer=_findFooter(panel);
  ...
  let footerPH=null; if(footer && footer.parentNode!==panel){ footerPH={ parent:footer.parentNode, next:footer.nextSibling }; }
  ...
  frameEl.replaceChildren(panel); activePanel=panel;
  ...
  const aspect=_measureAspect(body); const forceSquare=Math.abs(aspect-1)<=SQUARE_EPS && !isGrid;
  const box=_contentBox(header, footer, panel);
  ...
  try{ panel.dispatchEvent(new CustomEvent('toy-zoom',{detail:{zoomed:true},bubbles:true})); }catch{} try{ window.dispatchEvent(new Event('resize')); }catch{}
  const settle=()=>{
    const b1=_contentBox(header, footer, panel);
    const s=_fitBody(aspect, b1, forceSquare, panel);
    _applyLayout(panel, header, body, footer, s.w, s.h);
    const br=body.getBoundingClientRect();
    _diag('settlePass', { box:b1, size:s, bodyRect:{w:br.width|0, h:br.height|0} });
  };
  requestAnimationFrame(()=>{ settle(); requestAnimationFrame(settle); setTimeout(settle, 60); });
  const relayout=()=>{ settle(); _diag('relayout', { reason:'resize/observer' }); };
  relayoutHandler=relayout; addEventListener('resize', relayout, {passive:true});
  if(window.visualViewport){ try{ visualViewport.addEventListener('resize', relayout, {passive:true}); }catch{} try{ visualViewport.addEventListener('scroll', relayout, {passive:true}); }catch{} }
  try{
    if(roFrame) roFrame.disconnect(); roFrame=new ResizeObserver(relayout); roFrame.observe(frameEl);
    if(roParts) roParts.disconnect(); roParts=new ResizeObserver(relayout); if(header) roParts.observe(header); if(footer) roParts.observe(footer);
    if(roBody) roBody.disconnect(); roBody=new ResizeObserver(relayout); roBody.observe(body);
  }catch{}
  escHandler=(ev)=>{ if(ev.key==='Escape') zoomOutPanel(panel); }; addEventListener('keydown', escHandler, {passive:true});
}
export function zoomOutPanel(panel){
  ...
  try{
    ...
    if(relayoutHandler){
      ...
      if(window.visualViewport){ try{ visualViewport.removeEventListener('resize', relayoutHandler); }catch{} try{ visualViewport.removeEventListener('scroll', relayoutHandler); }catch{} }
    }
    try{ if(roFrame){ roFrame.disconnect(); roFrame=null; } }catch{} try{ if(roParts){ roParts.disconnect(); roParts=null; } }catch{} try{ if(roBody){ roBody.disconnect(); roBody=null; } }catch{}
    ...
    try{ overlayEl.style.display='none'; }catch{} try{ frameEl.replaceChildren(); }catch{} activePanel=null; restoreInfo=null;
  }
}
