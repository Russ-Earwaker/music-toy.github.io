...
const perfLabVersion = '2026-06-25-pickup-rocket-orbit-wide-v9';
import(`./perf/perf-lab.js?v=${perfLabVersion}`).catch((err) => {
  try { console.warn('[main] perf lab import failed', err); } catch {}
});
import './beat-swarm/weapon-gate-lab.js?v=2026-06-18-corridor-curve-v1';
import './toy-layout-manager.js';
import './zoom-overlay.js';
import './toy-spawner.js';
import { BeatSwarmMode } from './beat-swarm/beat-swarm-mode.js?v=2026-06-25-pickup-rocket-orbit-wide-v9';
    ...
    return [
        {
            type: 'beat-swarm',
            name: 'Beat Swarm',
            description: 'Pilot a center-locked ship through the scene.',
            action: () => BeatSwarmMode.enter(),
        },
    ];
}
...
try {
    ...
    if (window.ToySpawner && typeof window.ToySpawner.configure === 'function') {
        ...
        if (typeof window.ToySpawner.configureGame === 'function') {
            window.ToySpawner.configureGame({
                getCatalog: () => getGameCatalog(),
            });
        }
    }
} catch (err) {
