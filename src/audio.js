// src/audio.js (consolidated: CSV loader + legacy tones + master gain)
...
export function setMasterVolume(v){
  ...
  try { g.gain.setTargetAtTime(masterMuted ? 0 : masterVolume, ac.currentTime, 0.01); }
  catch { g.gain.value = masterMuted ? 0 : masterVolume; }
}
...
// UI event listeners (from toyui)
if (typeof window !== 'undefined'){
  window.addEventListener('master-volume', (e)=>{ try { setMasterVolume(e?.detail?.value); } catch{} });
  window.addEventListener('master-mute',   (e)=>{ try { setMasterMuted(!!(e?.detail?.muted)); } catch{} });
}
...
function playKeypadAt(freq, when){
  ...
  bp.connect(g).connect(getMasterGain());
...
function playPopAt(freq, when){
  ...
  noise.connect(bp).connect(g).connect(getMasterGain());
...
function playPadAt(freq, when){
  ...
  osc.connect(lp).connect(g).connect(getMasterGain());
...
function playAlienAt(freq, when){
  ...
  carrier.connect(g).connect(getMasterGain());
...
export function createScheduler(scheduleCallback, onLoop){
  ...
  return {
    async start(){
      ...
      // iOS unlock blip
      const g = ctx.createGain(); g.gain.value = 0.0001; g.connect(getMasterGain());
