export function ensurePauseWeaponUiRuntime(ctx) {
  ...
  pauseScreenEl.addEventListener('click', (ev) => {
    ...
    if (action === 'open-music-theme') {
      openPlayerMusicThemeEditor(String(actionEl.dataset.themeId || ''));
      return;
    }
  ...
  pauseScreenEl.addEventListener('pointerdown', (ev) => {
    const stagePickerState = getStagePickerState();
    const tuneEditorState = getTuneEditorState();
    const musicThemeEditorState = getMusicThemeEditorState();
    const weaponSubBoardState = getWeaponSubBoardState();
    const pauseWeaponDrag = getPauseWeaponDrag();
    if (!getPauseScreenEl() || stagePickerState.open || tuneEditorState.open || musicThemeEditorState.open || weaponSubBoardState.open) return;
...
export function renderPauseWeaponUiRuntime(ctx) {
  ...
  const weaponLoadout = getWeaponLoadout();
  const visibleWeaponLoadout = weaponLoadout.slice(0, 1);
  const stagePickerState = getStagePickerState();
  const cards = visibleWeaponLoadout.map((slot, slotIndex) => {
    ...
    return `
      <section class="beat-swarm-weapon-card${getPreviewSelectedWeaponSlotIndex() === slotIndex ? ' is-preview-selected' : ''}" data-slot-index="${slotIndex}">
        ...
        <div class="beat-swarm-weapon-toolbar">
          ...
          <div class="beat-swarm-weapon-actions" aria-label="${slot.name} controls">
  ...
  const playerMusicThemes = typeof getPlayerMusicThemes === 'function' ? getPlayerMusicThemes() : {};
  const musicThemeOrder = ['leadTheme', 'bassDrive', 'accentRhythm', 'powerTheme'];
  const musicThemeCards = musicThemeOrder.map((themeId) => {
    const theme = playerMusicThemes?.[themeId];
    if (!theme) return '';
    const toyType = String(theme.toyType || '').replace(/([a-z])([A-Z])/g, '$1 $2');
    const stateLabel = theme.autogenerated ? 'Generated starter' : 'Edited';
    return `
      <button type="button" class="beat-swarm-music-theme-btn" data-action="open-music-theme" data-theme-id="${themeId}">
        <span class="beat-swarm-music-theme-label">${theme.label || themeId}</span>
        <span class="beat-swarm-music-theme-detail">${toyType || 'Theme'} | ${stateLabel}</span>
      </button>
    `;
  }).join('');
  const musicThemesMarkup = musicThemeCards ? `
    <section class="beat-swarm-music-themes" aria-label="Player music themes">
      <div class="beat-swarm-music-themes-head">
        <div class="beat-swarm-music-themes-title">Music Themes</div>
        <div class="beat-swarm-music-themes-summary">Player-authored lanes for later runtime integration.</div>
      </div>
      <div class="beat-swarm-music-theme-row">${musicThemeCards}</div>
    </section>
  ` : '';
  const selectedSlot = getPreviewSelectedWeaponSlotIndex();
  const previewStatus = Number.isInteger(selectedSlot) && selectedSlot < visibleWeaponLoadout.length
